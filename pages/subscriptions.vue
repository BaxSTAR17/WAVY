            ...
            <div v-else class="w-full h-content flex flex-col">
                <NuxtLink v-for="(sub, index) in subscripts" :to="`/profile/${sub[0].UserID}`" class="rounded-xl bg-neutral-300 dark:bg-neutral-700 h-content border-box p-3 w-full flex justify-start items-center gap-2">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <NuxtLink :to="`/profile/${sub[0].UserID}`" class="rounded-xl bg-purple-800 hover:bg-purple-900 text-center w-content border-box p-2 h-content flex items-center gap-3"><UIcon name="i-uil-user-minus" size="25"/>UNSUBSCRIBE</NuxtLink>
                        </UTooltip>
                    </div>
                </NuxtLink>
            </div>
