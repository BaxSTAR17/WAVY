    ...
    <main class="h-screen w-screen flex flex-row">
        ...
        <div class="bg-neutral-800 min-h-dvh p-5 w-120 flex flex-col justify-center items-center">
            <form @submit.prevent="register()" id="form" class="w-100 flex flex-col gap-2 items-start justify-center">
                <h1 v-if="verifying" class="w-full self-center text-center text-4xl flex flex-col items-center">Please check your email to verify your account<UIcon name="i-uil-fast-mail" size="100"/></h1>
                <h3 class="m-0" v-if="!verifying">Username</h3>
                ...
                <input name="username" v-if="!verifying" type="text" placeholder="podcastLover17" v-model="username" class="h-9 bg-[#4e4b55] rounded-3xl mb-6 p-3 w-full">
                <h3 v-if="!verifying" class="m-0">Email</h3>
                <input name="email" v-if="!verifying" type="email" placeholder="email@RGB.com" v-model="email" class="h-9 bg-[#4e4b55] rounded-3xl p-3 w-full">
                <h3 v-if="!verifying" class="m-0">Password</h3>
                <input name="password" v-if="!verifying" type="password" autocomplete="new-password" placeholder="pass123" v-model="pass" class="h-9 bg-[#4e4b55] rounded-3xl p-3 w-full">
                <h3 class="m-0" v-if="!verifying">Confirm Password</h3>
                <input name="confirm" v-if="!verifying" type="password" autocomplete="new-password" placeholder="pass123" v-model="pass2" class="h-9 bg-[#4e4b55] rounded-3xl p-3 w-full">
            ...
            <div class="p-3 mt-6 text-white w-full cursor-pointer flex items-center justify-center">
                <span class="flex">Already have an account? <NuxtLink to="./login" class="text-purple-400">&nbsp;Log In Instead!</NuxtLink></span>
            </div>
